...
class Stock{
	...
	public static function getStock( $warehouseId, $cartonId=null, $categoryId=null, $paletteId=null, $locationId=null, $male=null, $female=null, $baby=null, $summer=null, $winter=null, $showEmpty=false ){
		
		if( $warehouseId && ($cartonId || $categoryId || $paletteId || $locationId) ){
		
			$sql = "SELECT carton, category, (income-outgo) AS amount, income, outgo, male, female, baby, summer, winter FROM ".Database::getTableName('stock')." WHERE ";
			$where = "";
			$attributes = '';
			$data = array();
			...
			$sql = $sql.$where." GROUP BY category, male, female, baby, summer, winter";
	...
	public static function addArticle( $cartonId=0, $categoryId=0, $male=false, $female=false, $baby=false,
		...
		$sql = "SELECT COUNT(carton) as count FROM ".Database::getTableName('stock')
				." WHERE carton=? AND category=? AND male=? AND female=? AND baby=? AND winter=? AND summer=?";
		$response = Database::getInstance()->sql( 'getArticle', $sql, 'iiiiiii', [
				$cartonId,
				$categoryId,
				$male,
				$female,
		...
		if( is_array($response) && $response[0]['count'] > 0 ){
			...
			$sql = "UPDATE ".Database::getTableName('stock')." SET income=income+?, outgo=outgo+?"
					." WHERE carton=? AND category=? AND male=? AND female=? AND baby=? AND winter=? AND summer=?";
			return Database::getInstance()->sql( 'editArticle', $sql, 'iiiiiiiii', [
					$incomeAdd,
			...
			// new entry
			$sql = "INSERT INTO ".Database::getTableName('stock')." (income, outgo, carton, category, male, female, baby, winter, summer)"
					." VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
			return Database::getInstance()->sql( 'editArticle', $sql, 'iiiiiiiii', [
					$incomeAdd,
					($outgoAdd < 0 ? $outgoAdd*(-1) : $outgoAdd ),
